...
def test_batch_sizes(input_df, pdb_folder, test_sizes=[10, 50, 100, 200, 500], test_rows=1000):
    """Test different batch sizes to find optimal performance"""
    print(f"Testing batch sizes with {test_rows} rows...")
    
    # Read first test_rows rows
    df = input_df[:test_rows]
...
def process_dataframe_parallel(input_file, output_file, pdb_folder, n_processors=None, batch_size=None, optimize_batch=True):
    ...
    try:
        ...
        if optimize_batch:
            ...
            # send a copy of the dataframe to test_batch_sizes
            batch_size = test_batch_sizes(df.copy(), pdb_folder)            
