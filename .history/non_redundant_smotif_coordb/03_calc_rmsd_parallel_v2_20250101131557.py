...
def create_sorted_heatmapold(rmsd_matrix, pdb_list, smotif_type, smotif_output_dir):
    ...
    # Replace NaN with max value for clustering
    matrix_for_clustering = df.fillna(df.max().max()).values
    condensed_dist = squareform(matrix_for_clustering)
    
    # Perform clustering
    linkage = hierarchy.linkage(condensed_dist, method='average')
    
    # Get the order of items after clustering
    ...
    return sorted_df.index.tolist()  # Return sorted PDB list for redundant groups
