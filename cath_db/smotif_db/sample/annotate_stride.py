...
def process_stride_file(stride_file):
    ...
    and applying minimum length requirements for Smotifs.
            ...
            if line.startswith('ASG'):
                ...
                if simple_type in ['H', 'E']:
                    if current_sse and current_sse.type == simple_type:
                        # Extend current SSE
                        current_sse = SSE(simple_type, current_sse.start, residue_num, current_sse.length + 1)
                    elif current_sse and current_sse.type == 'H' and is_helix(ss_type):
                        # Merge adjacent helical segments
                        current_sse = SSE('H', current_sse.start, residue_num, current_sse.length + 1)
                    else:
                        # Start new SSE
                        if current_sse and meets_length_requirement(current_sse):
                            sses.append(current_sse)
    ...
    for i in range(len(sses) - 1):
        sse1 = sses[i]
        sse2 = sses[i + 1]
        # Only create pairs if both elements are helices or strands and meet length requirements
        if (sse1.type in ['H', 'E'] and sse2.type in ['H', 'E'] and 
            meets_length_requirement(sse1) and meets_length_requirement(sse2)):
