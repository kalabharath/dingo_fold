...
class SmotifDataset(Dataset):
    ...
    def _create_aa_vocab_old1(self):
...
class SmotifTransformer(nn.Module):
    def __init__(self, vocab_size, num_smotifs, d_model=768, nhead=16, num_encoder_layers=8, dim_feedforward=3072, dropout=0.2):
        super().__init__()
    ...
    def forward(self, src):
        src = self.embedding(src) * math.sqrt(768)
...
def predict_smotif_old(model, sequence, aa_vocab, smotif_vocab, max_seq_length=50, device=torch.device("cuda")):
    model.eval()
    with torch.no_grad():
...
def main():
    ...
    scheduler = OneCycleLR(optimizer, max_lr=0.001, epochs=100, steps_per_epoch=len(train_loader))
    ...
    # Example inference
    sequence = "YIAKQRQISFVKSHF<SEP>SRQLEERLLIEGLYTHMKAL"
