...
class SmotifTransformer(nn.Module):
    def __init__(self, vocab_size, num_smotifs, d_model=1024, nhead=16, num_encoder_layers=12, dim_feedforward=4096, dropout=0.3):
        super().__init__()
    ...
    def forward(self, src):
        src = self.embedding(src) * math.sqrt(1024)
        ...
        output = torch.sum(output * attention_weights.unsqueeze(-1), dim=1)
...
def train_epoch(model, dataloader, criterion, optimizer, scheduler, device, max_grad_norm=1.0):
    model.train()
    total_loss = 0
...
def predict_smotif(model, sequence1, sequence2, aa_vocab, smotif_vocab, max_seq_length=50, device=torch.device("cuda")):
    model.eval()
    with torch.no_grad():
        seq = (sequence1 + '<SEP>' + sequence2)[:max_seq_length]
...
def main():
    ...
    scheduler = OneCycleLR(optimizer, max_lr=0.005, epochs=100, steps_per_epoch=len(train_loader), pct_start=0.1)
    ...
    sequence1 = "YIAKQRQISFVKSHF"
    sequence2 = "SRQLEERLLIEGLYTHMKAL"
    predicted_smotif = predict_smotif(model, sequence1, sequence2, full_dataset.aa_vocab, full_dataset.smotif_vocab, device=device)
    logger.info(f"Predicted Smotif ID for sequence '{sequence1}<SEP>{sequence2}': {predicted_smotif}")
